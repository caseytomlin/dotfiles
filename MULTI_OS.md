...
| Git remote / SSH helper | Same split: `.sh.tmpl` on Linux, `.ps1.tmpl` on Windows. **WSL** forces GitHub **HTTPS** (SSH `:22` times out on VPN/NAT); Windows may still rewrite to SSH. |
